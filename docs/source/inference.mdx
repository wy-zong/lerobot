...
| `Right Arrow` | Save current episode and start the next     |
| `Left Arrow`  | Discard current episode and re-record it    |
| `Enter`       | Push dataset to Hub (corrections-only mode) |
| `ESC`         | Stop the session                            |

`Left Arrow` only discards the current in-progress episode buffer. It does not delete episodes
that have already been saved to disk. In continuous recording mode, the current episode may
contain both autonomous and correction frames, and `Left Arrow` discards both.
