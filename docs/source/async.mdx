...
3. How to tune key parameters (`actions_per_chunk`, `inference_mode`, `chunk_size_threshold`) for your robot and policy.
...
python -m lerobot.async_inference.robot_client \
    ...
    --inference_mode=async \ # CLIENT: async overlaps action chunks; sync waits for the current chunk to finish
...
- `actions_per_chunk`, `inference_mode`, and `chunk_size_threshold` are key parameters to tune for your setup.
...
The following three parameters are key in every setup:
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>async</td>
      <td>
        <code>async</code> requests the next chunk before the current queue is empty.
        <code>sync</code> waits until the current chunk is fully executed before
        sending the next must-go observation.
      </td>
    </tr>
    <tr>
      <td>
...
This option does also put more pressure on the inference pipeline, as a consequence of the many requests. Conversely, values of `chunk_size_threshold` close to 0.0 make async mode more sequential, but the client may still have an in-flight remote inference request.

Use `inference_mode="sync"` when you want strict chunk-by-chunk behavior over two hosts: the robot client sends a must-go observation only after its local action queue is empty, waits for the remote `PolicyServer` to return the next chunk, and sends no new action while it is waiting. This is useful when you want inference to run on a faster remote GPU while avoiding overlap and action aggregation effects. The idle time is roughly the network round trip plus server inference time; for SmolVLA on a 4090-class GPU this is often around 0.1-0.2 seconds.
...
3. **Choose `inference_mode`**.
   - Use `async` to keep the robot moving while the server predicts the next chunk. This is the default and uses `aggregate_fn_name` to merge overlapping action chunks.
   - Use `sync` to wait for each chunk to finish before requesting the next one. In this mode, `chunk_size_threshold` and action aggregation do not provide overlap benefits.
4. **Adjust `chunk_size_threshold`**.
...
- **Tunable Parameters**: Success depends on properly configuring `actions_per_chunk`, `inference_mode`, and `chunk_size_threshold` for your specific hardware, policy, and task requirements
