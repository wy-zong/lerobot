...
class DatasetReader:
    ...
    def __init__(
        ...
        Args:
            ...
            image_transforms: Optional torchvision v2 transform applied to
                visual features.
            skip_video_decode: if True, skip video decoding and return placeholder tensors.
        """
        self._meta = meta
        self.root = root
    ...
    def _query_videos(self, query_timestamps: dict[str, list[float]], ep_idx: int) -> dict[str, torch.Tensor]:
        ...
        if self._skip_video_decode:
            # SARM can use precomputed CLIP features indexed by frame instead of decoded frames.
            return {
                vid_key: torch.zeros(len(query_ts), 1, 1, 1) for vid_key, query_ts in query_timestamps.items()
            }
